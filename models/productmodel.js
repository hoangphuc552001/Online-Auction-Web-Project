...
export default {
    ...
    async searchFullText(proName, limit, offset) {
        const sql = `select *
                     FROM product
                     WHERE MATCH (name) AGAINST("${proName}")
                        or MATCH (description) AGAINST("${proName}")
                         LIMIT ${limit}
                     OFFSET ${offset}`
    ...
    async totalSearchFullText(proName) {
        const sql = `select count('product.id')
                     FROM product
                     WHERE MATCH (name) AGAINST("${proName}") or MATCH (description) AGAINST("${proName}")`
        const raw_data = await db.raw(sql)
        return raw_data[0]
    }
    ,
    ...
    async bid(entity) {
        const sql = `select history.offer as price,
                            bidder.name   as bidder,
                     ...
                     from history,
                          ...
                          product,
                          user as seller
                     where history.product = ${entity.product}
                       and history.user = ${entity.user}
                       and product.id = history.product
                       and bidder.id = history.user
                       and product.seller = seller.id
                     order by history.offer desc limit 1`
        const raw_data = await db.raw(sql)
        return raw_data[0]
    },
    ...
    countLikeBidder(id, like) {
        return db("rating").count('id as count').where({
            "rating.seller": id,
            "rating.like": like,
    ...
    updateTimePro(id) {
        return db('product').update('status', "sold").where({
            'id': id,
            'status': 'bidding'
        })
    },
    expiredtimePro(id) {
        return db('product').update('status', "123123123123").where({
            'id': id,
            'status': 'bidding'
        })
    },
