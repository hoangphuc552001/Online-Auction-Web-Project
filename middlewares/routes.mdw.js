...
export default function (app) {
  app.get("/", async function (req, res) {
    ...
    for (let i=0;i<highestprice.length;i++){
      const diffDaysHighest = Math.round(Math.abs((highestprice[i].end - highestprice[i].start) / oneDay));
      ...
      +Math.floor(diffMshighest / 86400000)*24*60+Math.floor((diffMshighest % 86400000) / 3600000)*60; // minutes
    ...
    for (let i=0;i<mostbids.length;i++){
      const diffDaysMostBids = Math.round(Math.abs((mostbids[i].end - mostbids[i].start) / oneDay));
     ...
     var checkMinuteMostBids=
          ...
          +Math.floor(diffMsMostBids / 86400000)*24*60+Math.floor((diffMsMostBids % 86400000) / 3600000)*60; // minutes
      mostbids[i].minute=checkMinuteMostBids<=15?1:0;
    }
    const instance = await productmodel.findTop5ProInstance();
    for (let i=0;i<instance.length;i++){
      const diffDaysInstance = Math.round(Math.abs((instance[i].end - instance[i].start) / oneDay));
      ...
      var checkMinuteMostInstance=
          ...
          +Math.floor(diffMsMostInstance / 86400000)*24*60+Math.floor((diffMsMostInstance % 86400000) / 3600000)*60; // minutes
